    ...
    public class CommandStorage : ICommandStorage
    {
        ...
        public void Add(ICommand command)
        {
            if (command != null)
            {
                ...
                if (ecom != null) {
                    AppLogger.Log("CommandID " + commandID +" is already registered by name: " + ecom.Name, LogType.Error);
                    return;
                }

                _cStorageByName.TryGetValue(cName, out ecom);
                if (ecom != null) {
                    AppLogger.Log("CommandName " + cName + " is already registered by ID: " + ecom.UniqueID, LogType.Error);
                    return;
                }
